...
2. 确认 saves/{card}/rp-log.txt 存在
3. Bash 运行:
   python scripts/extract-img.py saves/{card}/rp-log.txt -g --latest-only  (最新一张)
   python scripts/extract-img.py saves/{card}/rp-log.txt -g               (全部)
4. 告知用户图片路径 (generated/{card}/)
...
| 类型 | 文件路径 | 更新频率 |
|------|----------|----------|
| project | saves/{card}/memory/project.md | 每轮自动 |
| reference | memory/reference.md | 几乎不变 (全局) |
| feedback | saves/{card}/memory/feedback.md | 用户指定时 |
| user | saves/{card}/memory/user.md | 低频 |

每轮结束后自动更新 saves/{card}/memory/project.md，保持不超过 300 字摘要。

# === 重要规则 ===
- 每轮 RP 回复后，用 Write 工具将本轮完整输出 (含 [img: ...]) 追加写入 saves/{card}/rp-log.txt
- 每轮 RP 结束后更新 saves/{card}/session-state.md: 当前场景、角色状态(所有已出场)、开放钩子、待生图标签
...
/play 流程:
  ...
  3. 检查 saves/{card}/ 是否存在
  4. 存在 → Read session-state.md → Read rp-log.txt 末 30 行 → 接续
  5. 不存在 → Read 角色卡/{card}.json → 初始化存档 → 从 first_mes 开始
...
| /branch | 切分支 |
| /loop | 循环执行 |
| /img 生成 | 从 saves/{card}/rp-log.txt 提取 [img: ...] 并调用 NAI 生图 |
...
3. Read saves/{card}/session-state.md
4. Read saves/{card}/rp-log.txt 的最后 30 行
