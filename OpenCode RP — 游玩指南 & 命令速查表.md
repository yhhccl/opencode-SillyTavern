...
AI 自动:
  ...
  2. 回扫正文 → 提取 [img: ...] 标签
  3. Write 追加到 角色卡/{card}/rp-log.txt
  4. Write 更新 角色卡/{card}/session-state.md
  5. Write 更新 角色卡/{card}/memory/project.md
...
AI 自动:
  ...
  - Read 角色卡/{card}/session-state.md → 场景全貌 (~400 token)
  - Read 角色卡/{card}/rp-log.txt 末 30 行 → 对话上下文 (~300 token)
  - 从 Next Direction 自然接续

继续 Web 模式:
  3. /web start
  4. /loop
  5. 浏览器: http://localhost:8765
...
A: `角色卡/{card}/` 目录中有备份。如果全丢了，重新 `/play {card}` 从开场开始。
...
| 命令 | 效果 |
|------|------|
| `/img 生成` | 从 角色卡/{card}/rp-log.txt 提取最新 [img: ...] 并生成 |
...
# 只提取不生成
python scripts/extract-img.py 角色卡/{card}/rp-log.txt
...
│   └── {card}/
│       ├── card.json     ← 角色卡本体
│       ├── worldbooks/   ← 世界书 (多本 JSON)
│       ├── rp-log.txt    ← 聊天记录 (运行时)
│       ├── session-state.md ← 状态快照 (运行时)
│       ├── generated/    ← NAI 生图输出
│       └── memory/
│           ├── project.md
│           ├── feedback.md
...
初次使用卡片:
  /play 巨根症
  → AI 读角色卡/{card}/card.json → 初始化角色卡/{card}/ 目录 → 从 first_mes 开始

恢复卡片:
  /play 巨根症 (或 /resume)
  → Read 角色卡/{card}/session-state.md
  → Read 角色卡/{card}/rp-log.txt 末 30 行
  → 自然接续
...
日常 RP (每轮):
  AI 写叙事 → 提取标签 → Write 角色卡/{card}/rp-log.txt
                  → Write 角色卡/{card}/session-state.md
```
初次使用卡片:
  /play 巨根症
